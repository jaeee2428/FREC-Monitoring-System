...
export default function DashboardCertStatus({
  ...
  const handleActionClick = (request) => {
    // TODO: Add another role here as necessary and make its version accordingly
    if (role === "student") {
        ...
        <div className="overflow-x-auto">
          ...
          {requests.map((request, index) => (
            <React.Fragment key={request.id}>
              <div className="grid min-w-[860px] grid-cols-[56px_140px_1.4fr_100px_150px_150px_150px] items-center border-t border-slate-100 px-5 py-4 first:border-t-0">
              ...
              {role === "student" && expandedRequestId === request.id && (
                <div className="border-t border-slate-100 pb-2">
                  <RequestProgress modeStr={request.modeStr} status={request.status} />
                </div>
              )}
            </React.Fragment>
          ))}
        ...
        <div className="grid grid-cols-1 gap-4 p-5 md:grid-cols-2 lg:grid-cols-3">
          {requests.map((request, index) => (
            <div key={request.id} className="flex flex-col rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              ...
              {request.type && (
                <p className="mb-3 text-xs text-slate-500">{request.type}</p>
              )}
              ...
              {role === "student" && expandedRequestId === request.id && (
                <div className="mt-4 border-t border-slate-100 pt-2">
                  <RequestProgress modeStr={request.modeStr} status={request.status} compact={true} />
                </div>
              )}
            </div>
          ))}
